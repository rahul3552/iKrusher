...
class ExpirationReminder
{
    ...
    /**
     * Find active quotes that are expired soon and send reminder email
     *
     * @throws LocalizedException
     */
    public function execute()
    {
        if (!$this->cronManagement->isLocked(Flag::AW_CTQ_PROCESS_EXPIRATION_REMINDER_LAST_EXEC_TIME)) {
            try {
                $this->quoteExpirationManagement->processExpirationReminder();
            } catch (\LogicException $e) {
                $this->logger->error($e);
            }
            $this->cronManagement->setFlagData(Flag::AW_CTQ_PROCESS_EXPIRATION_REMINDER_LAST_EXEC_TIME);
        }
    }
}
