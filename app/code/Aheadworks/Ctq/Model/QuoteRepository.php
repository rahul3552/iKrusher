...
class QuoteRepository implements QuoteRepositoryInterface
{
    ...
    public function get($quoteId)
    {
        if (!isset($this->registry[$quoteId])) {
            ...
            $this->registryByOrderId[$quote->getOrderId()] = $quote;
        }
    ...
    {
        if (!isset($this->registryByOrderId[$orderId])) {
            $quoteId = $this->resource->getIdByOrderId($orderId);
            if (!$quoteId) {
            ...
            $this->resource->load($quote, $quoteId);
            $this->registry[$quoteId] = $quote;
            $this->registryByOrderId[$orderId] = $quote;
        }
        return $this->registryByOrderId[$orderId];
    }
