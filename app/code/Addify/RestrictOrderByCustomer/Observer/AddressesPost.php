...
class AddressesPost implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $ships =$this->request->getParam('ship');
        $controller = $observer->getControllerAction();
        $newQtyArray = array();
        foreach ($ships as $key => $ship) {
            foreach ($ship as $keys => $shipitem) {
                $qty = !empty($shipitem['qty'])?$shipitem['qty']:1;
                            if(isset($newQtyArray[$keys])){
                                $newQtyArray[$keys] =$newQtyArray[$keys] +$qty ;
                            }else {
                            
                            $newQtyArray[$keys] =$qty ;
                            }
            }
        }


        foreach ($newQtyArray as $key => $qty) {
            $item = $this->itemCollection->create()->addFieldToFIlter('quote_id',$quote->getId())->addFieldToFIlter('item_id',$key)->getFirstItem();
                    ...
                    if ( in_array($customerId, $availableCustomerIds) || $groupCheck) {
                        if (in_array($product, $availableProductIds) || $restrictorderCollection->getProductType()==1) {
                            if ($qty < $restrictorderCollection->getMinQty()):
                                ...
                                $this->redirect->redirect($controller->getResponse(), 'multishipping/checkout/addresses');
                                return;
