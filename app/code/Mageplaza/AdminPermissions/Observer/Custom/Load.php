...
class Load implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if (strpos(get_class($object), Role::class) !== false
            || strpos(get_class($object), 'AdminPermissions') !== false
        ) {
            return;
        }
        ...
        if (!empty($customData)) {
            foreach ($customData as $datum) {
                if (!$datum['status'] || $datum['type'] !== 'model' || $datum['action'] !== 'load') {
                    continue;
                }
                if ($datum['class'] === trim(rtrim(get_class($object), 'Interceptor'), '\\')) {
                    throw new LocalizedException(__('You don\'t have permission to load model'));
                }
            }
        }
    }
