    ...
    public function execute(Observer $observer)
    {
        ...
        if (!empty($customData)) {
            foreach ($customData as $datum) {
                if (!$datum['status'] || $datum['type'] !== 'model') {
                    continue;
                }
                if ($datum['class'] === trim(rtrim(get_class($object), 'Interceptor'), '\\')) {
                    if (($datum['action'] === 'new') && !$object->getId()) {
                        throw new LocalizedException(__('You don\'t have permission to create model'));
                    }
                    if ($datum['action'] === 'save' && $object->getId()) {
                        throw new LocalizedException(__('You don\'t have permission to save model'));
                    }
                }
            }
        }
